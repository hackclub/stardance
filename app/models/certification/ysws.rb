...
module Certification
  class Ysws < ApplicationRecord
    ...
    # Columns the admin queue search matches as a substring. Each is backed by a
    # GIN trigram index, so an unanchored ILIKE stays an index scan.
    ...
    # Admin queue lookup. Text matches the project, its URLs, and the submitter's
    # identity. A bare number additionally matches a review or project id, since
    # that's almost always an id pasted out of Slack or the audit log — the
    # substring clauses still apply, so "123" also finds a project called
    # "Level 123".
    #
    # Email is matched as a prefix and Slack ID exactly, so both ride the btree
    # indexes that already exist on users rather than needing trigram indexes of
    # their own.
    scope :search, ->(term) {
