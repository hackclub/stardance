class AddInstagramToMaterializedAllSignups < ActiveRecord::Migration[8.1]
  # Surface the two largest acquisition channels that were invisibly buried in
  # `source_group = 'other'` in the canonical reporting matview: Instagram and
  # the Hack Club email campaign (ref = hc-email).
  #
  # Instagram was never normalized: the raw ref/user_ref values arrive as a long
  # tail ("instagram", "insta", "ig", "instagram reel(s)", "on instagram", URL
  # pastes, misspellings like "instragram"/"instgram", etc.). The old definition
  # kept each raw value as its own `primary_referral_source`, and `source_group`
  # had no `instagram` case, so every Instagram sign-up collapsed into
  # `source_group = 'other'` (~9% of all sign-ups).
  #
  # hc-email (the largest single signal in the whole dataset, ~17%) is a clean
  # `ref` URL param from Hack Club's email-campaign links. It already mapped to
  # `primary_referral_source = 'hc-email'` but, like Instagram, had no
  # `source_group` case, so it too fell into 'other'.
  #
  # This change:
  #   1. Folds the Instagram long tail into a single canonical
  #      `direct_referral_source = 'instagram'` (after the ambassador `a-%` /
  #      raffle `r-%` / github / amd branches, so those keep precedence). The
  #      detector is intentionally precise: `%insta%` and `ins[a-z]{0,3}gram`
  #      never match "telegram"/"program" (neither contains "insta" nor starts
  #      with "ins"), and `ig` is matched only as a whole word so it can't catch
  #      ambassador codes like `a-igazd`.
  #   2. Normalizes hc-email to `direct_referral_source = 'hc_email'` (snake_case,
  #      consistent with the other group keys).
  #   3. Adds `instagram` and `hc_email` buckets to `source_group` so both leave
  #      `'other'`.
  #
  # Consolidating the Instagram variants also feeds the deterministic
  # assumed-source distribution, so a proportional share of no-signal rows is now
  # attributed to Instagram as well (see primary_referral_methodology to
  # distinguish observed vs assumed).
  #
  # Materialized views can't be CREATE OR REPLACE-d and the prior definition
  # (20260608175849) already ran in production, so we drop and recreate. The only
  # changes from that migration are the Instagram + hc_email bits above; the
  # banned-email anti-join and the Linus-Tech-Tips => AMD mapping are preserved.
  # See 20260608132316 for the full rationale on why this matview lives outside
  # schema.rb.
  def up
    recreate_view!(with_new_categories: true)
  end

  def down
    recreate_view!(with_new_categories: false)
  end

  private

  def recreate_view!(with_new_categories:)
    direct_branches = if with_new_categories
      <<~SQL.strip
    ...
    source_group_cases = if with_new_categories
      <<~SQL.strip
    ...
    new_categories_comment = if with_new_categories
      <<~SQL.strip
