class RestoreLttOverrideInMaterializedAllSignups < ActiveRecord::Migration[8.1]
  # Restore the raw Linus-Tech-Tips => AMD override that 20260618120000 silently
  # regressed when it recreated this matview to split Instagram / hc-email out of
  # 'other'. AMD sponsored the LTT placement, so the canonical rule (established in
  # 20260610172657) is: anyone who self-reports "Linus Tech Tips" ANYWHERE in
  # raw_user_refs is credited to AMD at primary_referral_source, overriding the
  # /ref last-touch channel. 20260618120000 dropped that top-of-CASE raw_user_refs
  # match and reverted to the narrow `coalesce(first_ref, first_user_ref) = 'linus
  # tech tips'` equality, so LTT self-reporters who arrived via another channel
  # (program_page, hc-email, gh-edu, raffle/ambassador refs, ...) stopped counting
  # as AMD -- under-counting AMD by ~150 sign-ups.
  #
  # This recreates the matview from the current (20260618120000) definition and
  # only restores the LTT bits:
  #   - the raw_user_refs LTT branch at the TOP of direct_referral_source (ahead of
  #     ambassador / raffle / github / instagram / hc_email), replacing the narrow
  #     equality branch; and
  #   - the primary_referral_methodology branch explaining the override.
  # Everything else from 20260618120000 is preserved verbatim: the Instagram and
  # hc_email normalization + source_group buckets, the banned-email anti-join, the
  # unique index on (email) (required for REFRESH ... CONCURRENTLY), and the
  # simple `primary_referral_source = 'amd'` rule for amd_referral_group (every LTT
  # row now resolves to primary='amd', so the old raw_user_refs match there stays
  # redundant). actual_referral_source / known_referral_source still expose the raw
  # observed last-touch value for debugging.
  #
  # Side effect (intended, same as 20260610172657): folding LTT self-reports into
  # amd shifts the known-source distribution, so the deterministic assumed-source
  # assignment for no-signal rows moves a few rows into amd too.
  #
  # Materialized views can't be CREATE OR REPLACE-d and 20260618120000 already ran
  # in production, so we drop and recreate. See 20260608132316 for why this matview
  # lives outside schema.rb (the vanilla dumper ignores materialized views; only
    ...
    recreate_view!(
      direct_referral_source_case: <<~SQL.strip,
        case when lower(coalesce(raw_user_refs::text,'')) like '%linus tech tips%' then 'amd'
                   ...
                   when lower(coalesce(first_ref, first_user_ref)) like '%insta%'
                     or lower(coalesce(first_ref, first_user_ref)) ~ 'ins[a-z]{0,3}gram'
                     or lower(coalesce(first_ref, first_user_ref)) ~ '(^|[^a-z])ig([^a-z]|$)' then 'instagram'
                   when lower(coalesce(first_ref, first_user_ref)) = 'hc-email' then 'hc_email'
      ...
      SQL
      ltt_comment: <<~SQL.strip,
        'Anyone who self-reports ''Linus Tech Tips'' anywhere in raw_user_refs is '
  ...
  def down
    recreate_view!(
      ...
      SQL
      ltt_comment: <<~SQL.strip,
        'Sign-ups whose only signal is user_ref = ''Linus Tech Tips'' are credited to '
                  'AMD (amd) at the primary_referral_source level, since AMD sponsored that '
  ...
  def recreate_view!(direct_referral_source_case:, primary_referral_methodology_case:, ltt_comment:)
    safety_assured do
      ...
      select email, first_seen_at_utc, first_seen_day_et, seen_in_rsvps, seen_in_users, ip_address,
        ...
        case
          ...
          when primary_referral_source='instagram' then 'instagram'
          when primary_referral_source='hc_email' then 'hc_email'
          else 'other'
        end as source_group,
      ...
      execute <<~SQL
        COMMENT ON MATERIALIZED VIEW materialized_all_signups IS
          ...
          #{ltt_comment}
          'Instagram sign-ups are normalized into primary_referral_source = '
          '''instagram'' from the raw ref/user_ref long tail (instagram, insta, '
          'ig, instagram reels, misspellings, URL pastes, etc.); Hack Club '
          'email-campaign sign-ups (ref = hc-email) map to ''hc_email''. Both '
          'are rolled up via source_group. '
